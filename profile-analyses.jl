...
# ╔═╡ efe034bd-c2a5-4811-80ea-6ca23fdc77e3
begin
...
# ╔═╡ 39b913f8-ef33-45e3-9431-7de7f10de826
using StatsBase, ForwardDiff, LinearAlgebra

# ╔═╡ 061f0a7a-baa1-11ee-1bec-11e529031811
md"""
# Profile likelihoods and posteriors

0. Load Dani's data ✔
1. Set up model in Turing ✔
  - Need to range iteration over rows to iteration over fixations
  - Change to LogNormal instead of Gamma b/c bug ✔
2. Use conditioning syntax to set up "submodels" ✔
3. Maximize LL/log posterior for fixed values of each parameter ✔
4. Plot both profile likelihoods and posteriors ✔
5. Examine for identifiability
  - Still need to try out including ζ
  - The model, as is, seems structally non-identifiable.
  - But there is a pretty simple repameterization that seems much more identifiable
6. Calculate confidence intervals
  - Might not be that interesting, especially if the goal is Bayesian fitting anyway
7. Use results of profile analyses to guide Bayesian parameter fitting (init_params w/ Emcee, probably) ✔
8. Compare MCMC results to profiles

## Notes
- μ, r, and ν always only interact through multiplication, so they will clearly be redundant/non-identifiable
  - Solution: set r = 1 and allow ν to take any positive value
  - Allowing ν > 1 makes for an unituitive situation where words outside the fovea are processed much faster than words in the fovea, so a new σ(ν) fn. might be needed. Softmax?
- ι and κ only interact through division (albeit somewhat more complicated), ∴ shouldn't expect identifiability
  - Solution: we expect high activation and/or residual left saliency to slow processing at the current word, so we sum them and rescale the sum by ι and add it to μ.
"""

# ╔═╡ 66cfc6cf-98e8-4fb9-a5bf-055fc410718f
corpus = DataFrame(CSV.File("../simplification/cogsci2024/spacy-parses/spacy-parses-freq.txt", delim='\t'))

# ╔═╡ f6af7dac-a32a-47c1-afff-8389bb7e97b5
function normalizefreqs(freqs)
	lfreqs = log10.(freqs)
	return lfreqs ./ maximum(lfreqs)
end

# ╔═╡ 8059ea9c-d751-4c9e-a19a-e83254fa998b
corpus.nlogfreq = normalizefreqs(corpus.wfreq)

# ╔═╡ 391dca57-9b89-420a-8a71-b7db3ba128dc
data = CSV.read("../simplification/cogsci2024/englishdata/annotated_fixation_sequence.csv", delim=",", DataFrame)

# ╔═╡ 53d44834-4ad4-49ea-aaff-95c076b060c6
sort!(data, [:subject, :trial_id, :fixationno])

# ╔═╡ 684c4ef8-a77c-4093-91ae-d7baf0a3b9eb
length(unique(data.sentenceid)) / 4 # we only have the critical conditions

# ╔═╡ 46b2feab-a5f0-4690-b8a1-47b34d0b9f2b
sort(unique(data.sentenceid))

# ╔═╡ 579a0b09-b9d9-4e97-9e8a-2327866c50cb
sort(unique(data.subject))

# ╔═╡ f83c6e97-89c7-48bc-8c3f-0f4c56cca02e
length(unique(data.subject))

# ╔═╡ f9d1a5b3-28f4-4c29-9106-36058ef83d1a
length(unique(data[data.subject .== 1, :sentenceid]))

# ╔═╡ 5b0268d0-3b4e-42ab-a4e2-25eebc79dccd
combine(x -> length(unique(x.trial_id)), groupby(data, :subject))

# ╔═╡ 5e43a7f4-aabd-46a8-b223-d35405369cb9
combine(x -> length(unique(x.trial_id)), groupby(data, [:subject, :condition]))

# ╔═╡ 6f33b25c-a097-45c8-9df5-0a49b1a974b1
0.3*40

# ╔═╡ 26ab923d-ba9b-4f1e-aff1-d521c9d4b581
unstack(combine(groupby(data, :subject), x -> unique(x.sentenceid)), :subject, :x1, combine=vcat)

# ╔═╡ 97e00165-4210-4948-a74d-52ec19bef6b8
sort(unstack(data, :sentenceid, :condition, :sentenceid, combine=x -> length(unique(x))), :sentenceid)

# ╔═╡ 5cfd31d3-c2c3-4803-abb9-207cac195b63
unstack(data, :subject, :sentenceid, :subject, combine=x -> length(unique(x)))

# ╔═╡ 8431af3e-fe78-45c3-814f-e6dcf94e5dfa
...
# ╔═╡ d2ae0e4e-eb5c-4a2c-9378-29389cea638c
...
# ╔═╡ d1cebbce-1306-4e22-bc78-4f5cbfd29c6f
...
# ╔═╡ b02018a2-7fe1-4d70-bdb9-436573b32731
#histogram(rand(LogNormal(log(1.), 0.5), 1000) .- 0.5)
plot(LogNormal(log(200), 0.5))
#plot(Beta(3, 3))

# ╔═╡ 6c998cca-3da3-42bd-9407-dbddde6fccd3
begin
	plot(x -> inv(1 + 2*x + x^2), 0, 3, label="sigma")
	plot!(x -> x * inv(1 + 2*x + x^2), 0, 3, label="w±1")
	plot!(x -> x^2 * inv(1 + 2*x + x^2), 0, 3, label="w+2")#
end

# ╔═╡ c4ae21f5-b559-4c85-9ebb-89d6cd6b2e6e
@model function basemodel(corpus, data, fixatedwords, fixationdurations, ::Type{T} = Float64) where {T}
	# Priors
	...
	#ζ ~ Beta(1.5, 1.5)
	γ ~ truncated(Normal(1, 0.5), 0, Inf) # = one(T)
	...
	σ = inv(1 + 2*ν + ν^2)
	minsal = 10^η
	maxactivations = 1 .- β .* corpus.nlogfreq
	...
	Threads.@threads for pi in data
		...
		#deps = str_to_vec.(corpus[corpus.itemidseq .== itemnr, :deps])
		...
		for row in eachrow(@view pi[1:end-1, :])
			...
			#=
	        idx = findall(activations .>= maxact)
	        for i in idx
				cdep = deps[i]
				if not_yet_parsed[i] && !isempty(cdep)
					activations[cdep] .= ζ .* maxact[cdep]
					#activations[cdep] .= rand() .* maxact[cdep]
				end
	        end
			...
			leftsal = (nextword > 1) ? prod(1 .+ κ .* (saliencies[1:nextword-1]) .- minsal) : one(T)
			#leftsal = (nextword > 1) ? prod(saliencies[1:nextword-1] .- minsal; init=1.0) : zero(T)#one(T)
			#rate = (rate0 * (1 + ι * activations[nextword]) / leftsal)^-1
			#if rate <= 0.001#2*eps(typeof(ι))
			#	Turing.@addlogprob! -Inf
			#else
				#fixationdurations[rowidx+1] ~ Gamma(shapeparam, rate)
...
# ╔═╡ 0d81ca02-a09d-49d0-a7ff-720674168b53
# NOTE: https://github.com/JuliaStats/Distributions.jl/issues/1810
# Try replacing the Gamma likelihood by a LogNormal

# ╔═╡ 13999278-cd91-4f36-a165-ef6f560099df
gdata = groupby(data, [:subject, :trial_id]);

# ╔═╡ 7c711ed3-7b4e-49bf-91b3-4a9181f9b960
#forpost0 = basemodel(corpus, gdata, data.fixatedword[:], data.fixationduration[:]);
forpost0 = basemodel(corpus, gdata[1:12], data.fixatedword[:], data.fixationduration[:]);

# ╔═╡ 5bb0e082-b364-4924-9558-74ad871e339c
mle0 = optimize(forpost0, MLE(), LBFGS(); autodiff=:forward)#BFGS(initial_stepnorm=0.01))

# ╔═╡ 5f5bdf5b-1e56-4ac8-a0c2-d77ca090fa68
#map0 = optimize(forpost0, MAP())#, BFGS())

# ╔═╡ 7e659158-6bd5-42c3-ad29-3f1692ad3fbf
DynamicPPL.syms(DynamicPPL.VarInfo(forpost0))

# ╔═╡ 7f80ffcf-6f9c-477f-816a-d31a79f715ee
function profll(mod, var, vals, inits)
	mles = zeros(size(vals, 1))
	prevmle = zeros(length(inits))
	#maps = zeros(size(vals, 1))
	#prevmap = zeros(length(inits))
	#Threads.@threads
	for (i, v) in collect(enumerate(vals))
		if i == 1
			omle = optimize(fix(mod, (; var => v)), MLE(), inits; autodiff=:forward)
			mles[i] = omle.lp
			prevmle = omle.values.array
			#omap = optimize(fix(mod, (; var => v)), MAP(), inits)
			#maps[i] = omap.lp
			#prevmap = omap.values.array
		else
			omle = optimize(fix(mod, (; var => v)), MLE(), prevmle; autodiff=:forward)
			mles[i] = omle.lp
			prevmle = omle.values.array
			#omap = optimize(fix(mod, (; var => v)), MAP(), prevmap)
			#maps[i] = omap.lp
			#prevmap = omap.values.array
		end
	end
...
# ╔═╡ e64f6ab7-48b4-428f-ad0c-dca22264f5b8
...
# ╔═╡ 9e1a5eb8-5778-4164-aea4-d165c10d5911
begin
	nvals = 10
	vals = [range(100, 400, nvals), range(0, 1, nvals), range(-5, -0.01, nvals), range(0, 1, nvals), range(0.01, 20, nvals), #range(0, 1, nvals),
	range(0, 2, nvals), range(-1, 20, nvals), range(0, 10, nvals)]
	lls = allprofs(forpost0, vals, mle0.values.array)
end

# ╔═╡ 30f4139b-f404-41a9-bae0-80839d87bfd3
println("Full model took $(2187 / 60)min. (with 10 values for each parameter)")

# ╔═╡ 422297ea-3368-4963-8573-e1dc2ba055e5
function plotprofs(lls, vals)
...
# ╔═╡ aa5f0601-b186-44e8-8692-64046442708d
lls[:β]

# ╔═╡ d9243300-89ca-4e26-a55e-c59d47a30c32
begin
	pltpr = plotprofs(lls, vals)
	md"""
	$(pltpr)
	These likelihood profiles should be taken with a grain of salt: The optimitaion routine did not always converge, so the results could really just be guesses.
	"""
end

# ╔═╡ 8a487080-80f7-4537-9545-595a8bad1038
imat = informationmatrix(mle0)

# ╔═╡ 00f90772-aadb-4a70-b9fd-0209a816df33
isposdef(imat)

# ╔═╡ ee971cb8-577d-4559-9e68-75f44afe41b9
vcov(mle0)

# ╔═╡ f8e177c5-b71d-416c-b318-8aa4a27ef035
hess = ForwardDiff.hessian(mle0.f, mle0.values.array)

# ╔═╡ 7f8ec826-d374-429a-81dd-5c071029a907
rank(hess)

# ╔═╡ 28a97e7a-0478-434e-a0cb-98c23500247b
cond(hess)

# ╔═╡ 8479beb7-a6be-4a4c-967c-f1ac69216143
el, ev = eigen(hess)

# ╔═╡ c496f541-8a90-43f4-ac91-f318a19decf7
els = el ./ maximum(abs.(el))

# ╔═╡ 5c8e8913-929a-49eb-bafe-45eafce80728
# Number of estimable parameters
length(els[els .>= 0.001])

# ╔═╡ 6ab68879-cb80-44f0-92c0-d6ae3d07c0dd
md"""
## Zwischenfazit
Already from here, we can see that we shouldn't expect to get a lot of useful information from our likelihood profiles: The Hessian matrix of the likelihood function is nearly singular (rank ≪ nparams, condition ≈ ∞)
"""

# ╔═╡ d71a4991-79a7-4220-ad7c-46e5c7b43b10
#inits = repeat(map0.values.array, 1, 100)' .- 0.001 .* abs.(randn(100))

# ╔═╡ fbe38e37-ad27-4b57-9336-ccc60591de92
# ╠═╡ disabled = true
#=╠═╡
post = sample(forpost0, Emcee(50), 200)#; initial_params=[map0.values.array .- abs.(x) for x in 0.001 .* randn(20)])
#post = sample(forpost0, NUTS(), 10; initial_params=map0.values.array)
  ╠═╡ =#

# ╔═╡ d30e17ce-e7c9-41ee-8bd6-a82ee589014d
# ╠═╡ disabled = true
#=╠═╡
lj = logjoint(forpost0, post)
  ╠═╡ =#

# ╔═╡ a99633a0-9b3a-44d6-8c04-6f87f1946605
# ╠═╡ disabled = true
#=╠═╡
plot(lj)
  ╠═╡ =#

# ╔═╡ 3afd797b-8996-4e5d-adce-fe860d50d7f3
#=╠═╡
plot(post)
  ╠═╡ =#

# ╔═╡ e5bb27ae-28eb-422d-a37f-99187c80f8ce
#=╠═╡
pshort = post[100:end, :, :];
  ╠═╡ =#

# ╔═╡ 506e4dd0-2b8f-4c2a-a34d-a82d804b36cf
#=╠═╡
describe(pshort)
  ╠═╡ =#

# ╔═╡ 003b73aa-0abd-4ca1-9ec7-a89bb6873e20
#=╠═╡
begin
	plts = plot(layout = @layout([x _ _ _; x x _ _; x x x _; x x x x]), size=(800, 800))
	histogram2d!(plts[1, 1],pshort[:μ][:], pshort[:ν][:], bins=75, cbar=false, ylabel="ν", xticks=nothing)
	histogram2d!(plts[2, 1],pshort[:μ][:], pshort[:β][:], bins=75, cbar=false, ylabel="β", xticks=nothing)
	histogram2d!(plts[3, 1],pshort[:μ][:], pshort[:γ][:], bins=75, cbar=false, ylabel="γ", xticks=nothing)
	histogram2d!(plts[4, 1],pshort[:μ][:], pshort[:ι][:], bins=75, cbar=false, xlabel="μ", ylabel="ι")
	
	histogram2d!(plts[2, 2],pshort[:ν][:], pshort[:β][:], bins=75, cbar=false, xticks=nothing, yticks=nothing)
	histogram2d!(plts[3, 2],pshort[:ν][:], pshort[:γ][:], bins=75, cbar=false, xticks=nothing, yticks=nothing)
	histogram2d!(plts[4, 2],pshort[:ν][:], pshort[:ι][:], bins=75, cbar=false, xlabel="ν", yticks=nothing)

	histogram2d!(plts[3, 3],pshort[:β][:], pshort[:γ][:], bins=75, cbar=false, xticks=nothing, yticks=nothing)
	histogram2d!(plts[4, 3],pshort[:β][:], pshort[:ι][:], bins=75, cbar=false, xlabel="β", yticks=nothing)

	histogram2d!(plts[4, 4],pshort[:γ][:], pshort[:ι][:], bins=75, cbar=false, xlabel="γ", yticks=nothing)
end
  ╠═╡ =#
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "70d8e04780f356fb93efe02039ea4eccedbc89a3"
