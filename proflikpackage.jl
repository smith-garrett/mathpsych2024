...
# ╔═╡ ef7b13f3-c59a-4379-ad29-557ff6fcbbca
begin
...
# ╔═╡ 2caedd0f-aae3-4e0a-9f69-f5e0da79a51f
...
# ╔═╡ 51dd83d6-9e91-4de9-a9d6-001405bf41c1
...
# ╔═╡ f6032f5c-c860-4f8a-b094-51e13aa101ec
function _ll(p, data, corpus)
	# Priors/params
	ll = 0.0
	μ = p[1]
	#ll += logpdf(LogNormal(log(200), 0.5), μ)
	ν = 0.6#p[2]
	#ll += logpdf(Beta(1.5, 1.5), ν)
	η = -3.0#p[3]
	#ll += logpdf(truncated(Normal(-3, 0.5), -Inf, 0), η)
	β = p[2]
	#ll += logpdf(Beta(1.5, 1.5), β)
	r = 1.0#p[4]
	#ll += logpdf(LogNormal(log(7), 0.5), r)
	#ζ ~ Beta(1.5, 1.5)
	γ = 1.0 #p[6]
	#ll += logpdf(truncated(Normal(1, 0.5), 0, Inf), γ)
	ι = 0.0#p[7]
	#ll += logpdf(truncated(Normal(1, 0.5), -0.5, Inf), ι)
	κ = 0.0#p[8]
	#ll += logpdf(Exponential(5.0), κ)
	...
	Threads.@threads for pi in data
		...
		for (ri, row) in enumerate(eachrow(@view pi[1:end-1, :]))
			...
			# Spatial LL
			ll += logpdf(Categorical(probabilities), pi[ri+1, :fixatedword])
	
			# Temporal LL
			nextword = pi[ri+1, :fixatedword]
			leftsal = (nextword > 1) ? prod(1 .+ κ .* (saliencies[1:nextword-1]) .- minsal) : 1.0
			rate = rate0 * (1 + ι * activations[nextword]) / leftsal
			ll += logpdf(Gamma(shapeparam, inv(rate)), pi[ri+1, :fixationduration])
		end
	end
	return ll
end

# ╔═╡ 56f2f154-bdb2-4756-aa5b-40d3208c448f
#initguesses = [200.0, 0.3, -3, 0.6, 10.0, 0.1, 0.01, 0.1]
initguesses = [200.0, 0.3]#, 0.6, 10.0]

# ╔═╡ eb8a47ec-4ec4-4c58-90b5-af00332619da
_ll(initguesses, gdata, corpus)

# ╔═╡ 714f41e2-6ac3-4407-97c7-bd4e468e6968
ll(p, data) = _ll(p, gdata, corpus)

# ╔═╡ 5f9e6922-7ca1-4855-87f5-43bbfe27dd5c
ll(initguesses, gdata)

# ╔═╡ e284d068-7f4e-42b3-bc36-7f6a45502fdc
profprob = LikelihoodProblem(ll, initguesses; data=gdata, syms=[:μ, :β])#,ν :β, :r])

# ╔═╡ c704b5dd-d5e8-4cdd-b76e-e985a3e43883
#mleprof = mle(profprob, Optim.NelderMead())
mleprof = mle(profprob, NLopt.LN_COBYLA())

# ╔═╡ 70899be4-eb4e-4019-8bf0-ff9ba5c1f70a
begin
	lb = [0.0, 0]#, 0, 0]
	ub = [700.0, 1]#, 1, 20]
	resolutions = 100#[50, 10]
	param_ranges = construct_profile_ranges(mleprof, lb, ub, resolutions)
end

# ╔═╡ 2c3d077e-464f-4dda-86df-be24255e84b5
profres = profile(profprob, mleprof; param_ranges, parallel=true, min_steps=20)

# ╔═╡ 1d1d1e0e-ad64-4899-9843-20bdd8ac00bc
plot_profiles(profres;)# show_points=true)

# ╔═╡ 1cef20dc-8978-430b-ab77-39465de02cfd
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "6b51c232f6002ade0ccd40f1d37692564a09fe42"
